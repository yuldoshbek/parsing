...
**Результат на тестовом артикуле 246290927 (Оранжевый) — прогон подтверждён:**
- Было: 13 спарсено → 8 прошло (color-based + старый фильтр)
- Стало: 27 спарсено → 21 прошло (nmId-based + новый фильтр)
- Отсеяно только 3 — реально пустые: «все супер», «все отлично», «упаковано хорошо»
- Сентимент: было 57%/14%/29% → стало 86%/5%/10% (база выросла с 7 до 21 отзыва)

**Также исправлен баг filter_rate_pct:**
- Было: `1 - after_filter / working_base` (давал отрицательный % т.к. no-text 1★ тоже проходят Rule 1)
- Стало: `1 - text_base / working_base` (честный % отсева только среди текстовых)

**Отчёт:** `REPORT_v52_filter_fix.txt`
